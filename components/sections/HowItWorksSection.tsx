    ...
    <section ref={sectionRef} id="how-it-works" className="relative section-padding bg-[#0F0F0F]">
      ...
      <div className="container mx-auto px-4 md:px-6">
        ...
        <div className="max-w-6xl mx-auto grid lg:grid-cols-2 gap-8 items-start">
          <div ref={imageRef} className="relative h-72 md:h-[420px] rounded-3xl overflow-hidden">
            <Image
          ...
          <div className="glass-card rounded-3xl p-7 md:p-10">
            <ul ref={listRef} className="space-y-4 mb-8">
              {steps.map((step, index) => (
                <li key={step} className="flex items-start gap-4">
                  <span className="w-8 h-8 rounded-full bg-[#FFD400]/15 text-[#FFD400] text-sm font-black flex items-center justify-center shrink-0">
                    {index + 1}
                  </span>
                  <span className="text-white/75 leading-relaxed">{step}</span>
                </li>
              ))}
            </ul>
            ...
            <button
              onClick={openModal}
              className="w-full bg-[#FFD400] text-black font-black py-4 rounded-full hover:brightness-105 transition-all min-h-[48px]"
            >
